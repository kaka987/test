...
$(function () {
	...
	$(".user-search").click(function(){
		...
		var str = "";
		$.get("/usersearch?username="+username, function(data,status){
			var h = data.data[0];
			console.log(h);
	...
	$("#userform").submit(function() {
		...
		$.post("/useradd",user,function(data,status){
			console.log(data);
			if(data.status==1) {
				location.reload();
				return true;
...
function userEdit() {
	$("#userform-edit").submit(function() {
		...
		$.post("/useredit",user,function(data,status){
			console.log(data);
			if(data.status==1) {
				location.reload();
				return true;
...
function userDel() {
	$(".user-del").bind("click",function(){
		...
		if (confirm("Delete this user?")==true) {
			...
			$.post("/userdel",{"userid":userid},function(data,status){
				console.log(data);
				if(data.status==1) {
					location.reload();
				}
